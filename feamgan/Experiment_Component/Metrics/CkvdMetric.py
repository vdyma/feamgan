...
class CkvdMetric(KidMetric):
    def __init__(self, is_video, model_dir, dataset_name, dis_model_name="vgg16_f_ll"):
        super(CkvdMetric, self).__init__(
    ...
    def forwardBatch(self, real_data, fake_data, mode, save_real_prefix=None):
        ...
        if (
            save_real_prefix
            and os.path.exists(
                f"{self.save_path}/{save_real_prefix}_{mode}_{self.file_name}"
            )
            and (mode != "train")
        ):
            real_class_activations = None
        else:
            real_img_patched, real_seg_patched = self._oneEightCrop(
                real_data[0], real_data[1]
            )
            real_class_imgs = self._getClassImages(
                real_img_patched, real_seg_patched, p="r"
            )
            real_class_activations = self._getClassActivations(real_class_imgs)
            real_class_semantic_descriptions = self._getClassSemanticDescriptions(
                real_class_imgs, real_seg_patched
            )

        fake_img_patched, fake_seg_patched = self._oneEightCrop(
            fake_data[0], fake_data[1]
        )
        fake_class_imgs = self._getClassImages(
            fake_img_patched, fake_seg_patched, p="f"
        )
        fake_class_activations = self._getClassActivations(fake_class_imgs)
        fake_class_semantic_descriptions = self._getClassSemanticDescriptions(
            fake_class_imgs, fake_seg_patched
        )

        meter_key = f"{save_real_prefix}_{mode}"
        if meter_key not in self.meters:
            self.meters[meter_key] = {
                "fake_cls_des": {},
                "fake_cls_acts": {},
                "real_cls_des": {},
                "real_cls_acts": {},
        ...
        for cls in fake_class_activations:
            ...
            if cls not in self.meters[meter_key]["fake_cls_acts"]:
                self.meters[meter_key]["fake_cls_acts"].update(
                    {cls: [[] for _ in range(len(fake_class_activations[cls]))]}
                )
        for cls in real_class_activations:
            ...
            if cls not in self.meters[meter_key]["real_cls_acts"]:
                self.meters[meter_key]["real_cls_acts"].update(
                    {cls: [[] for _ in range(len(real_class_activations[cls]))]}
                )
        ...
        if not (
            ...
            for cls in real_class_activations:
                for i, real_act in enumerate(real_class_activations[cls]):
                    self.meters[meter_key]["real_cls_acts"][cls][i].append(
                        real_act.cpu()
                    )
                self.meters[meter_key]["real_cls_des"][cls].append(
                    real_class_semantic_descriptions[cls].cpu()
                )

        for cls in fake_class_activations:
            for i, fake_act in enumerate(fake_class_activations[cls]):
                self.meters[meter_key]["fake_cls_acts"][cls][i].append(fake_act.cpu())
            self.meters[meter_key]["fake_cls_des"][cls].append(
                fake_class_semantic_descriptions[cls].cpu()
            )
    ...
    def _getMatchingPairs(
        self, fake_des, fake_act, real_des, real_act, cls, sim_threshold=0.5
    ):
        ...
        for f in range(fake_des.shape[0]):
            ...
            if found_index > -1:
                for a in range(len(fake_act)):
                    fake_activation_paired[a].append(
                        np.expand_dims(fake_act[a][f], axis=0)
                    )
                    real_activation_paired[a].append(
                        np.expand_dims(real_act[a][found_index], axis=0)
                    )
            else:
                self.nr_no_matching_pairs[cls] += 1

        print(
            f"No matching pairs found for {self.nr_no_matching_pairs[cls]} samples for class {cls} "
        )
        ...
        if meter_key in self.meters:
            if save_real_prefix and os.path.exists(path) and (mode != "train"):
                ...
                if self.meters[meter_key]["real_cls_acts"]:
                    ...
                    for cls in real_cls_act:
                        ...
                        real_cls_act[cls] = [
                            torch.cat(act) for act in real_cls_act[cls]
                        ]
                        real_cls_act[cls] = [
                            distAllGatherTensor(act) for act in real_cls_act[cls]
                        ]
                        if isMaster():
                            real_cls_des[cls] = (
                                torch.cat(real_cls_des[cls]).cpu().data.numpy()
                            )
                            real_cls_act[cls] = [
                                torch.cat(act).cpu().data.numpy()
                                for act in real_cls_act[cls]
                            ]
                    if save_real_prefix:
            ...
            if self.meters[meter_key]["fake_cls_acts"]:
                ...
                for cls in fake_cls_act:
                    ...
                    fake_cls_act[cls] = [
                        distAllGatherTensor(act) for act in fake_cls_act[cls]
                    ]

                    if isMaster():
                        fake_cls_des[cls] = (
                            torch.cat(fake_cls_des[cls]).cpu().data.numpy()
                        )
                        fake_cls_act[cls] = [
                            torch.cat(act).cpu().data.numpy()
                            for act in fake_cls_act[cls]
                        ]
                        fake_activations_paired, real_activations_paired = (
                            self._getMatchingPairs(
                                fake_cls_des[cls],
                                fake_cls_act[cls],
                                real_cls_des[cls],
                                real_cls_act[cls],
                        ...
                        if fake_activations_paired and real_activations_paired:
                            for fake_activations, real_activations in zip(
                                fake_activations_paired, real_activations_paired
                            ):
