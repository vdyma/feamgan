...
def randomlySampleResults(
    ...
    random_index = np.random.randint(
        0, high=len(baseline_files) - 1, size=nr_random_samples
    )
    ...
    for model_name in model_names:
        ...
        if not os.path.exists(save_path):
            ...
            if include_baseline:
                os.makedirs(os.path.join(save_path, baseline))
        os.makedirs(os.path.join(save_path, model_name))

        for i, r_i in tqdm(
            zip(range(len(random_index)), random_index),
            desc=f"Randomly sampling results for {model_name} ...",
        ):
            shutil.copy(
                input_files_1[r_i], os.path.join(save_path, model_name, f"{i}.jpg")
            )
            if first_run:
                shutil.copy(
                    input_files_2[r_i], os.path.join(save_path, "source", f"{i}.jpg")
                )
                if include_baseline:
                    shutil.copy(
                        baseline_files[r_i],
                        os.path.join(save_path, baseline, f"{i}.jpg"),
                    )
        first_run = False
