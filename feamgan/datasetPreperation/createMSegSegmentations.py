...
def parseArguments():
    ...
    parser.add_argument(
        "--dataset_path",
        type=str,
        help="the dataset to create the mseg segmetnations for",
        nargs="?",
    ...
    parser.add_argument(
        "--rank",
        type=int,
        help="the rank of the current proccess (default: 0)",
        nargs="?",
        default=0,
        const=0,
    )
    parser.add_argument(
        "--skip_if_exists",
        type=bool,
        help="skipps the creation of segmentations if they already exist (default: 1)",
        nargs="?",
        default=True,
        const=True,
    )
    parser.add_argument(
        "--num_gpus",
        type=int,
        help="the number of gpus to devide the process (default: 1)",
        nargs="?",
        default=1,
        const=1,
    )
    parser.add_argument(
        "--prefix",
        type=str,
        help="the prefix for the split eg. 'daytime/' for the daytime folder in BDD100K (default: "
        ")",
        nargs="?",
        default="",
        const="",
...
def createMSegSegmentations(dataset_path, rank, num_gpus, skip_if_exists, prefix):
    ...
    for split in splits:
        ...
        if not os.path.exists(f"{dataset_path}/sequences/{split}"):
            continue
        file_paths = [
            x for x in sorted(glob.glob(f"{dataset_path}/sequences/{split}/frames/*"))
        ]  # For now we do it file by file because the e gpu runs out of memmory in the demo script if we use it on a entire folder
        file_paths = np.array_split(file_paths, num_gpus)

        for file_path in tqdm(
            file_paths[rank],
            total=len(file_paths[rank]),
            desc="Creating semantic segmentations",
        ):
            ...
            subprocess.check_call(
                [
                    f"python -u /workspace/unique_for_mseg_semantic/mseg-semantic/mseg_semantic/tool/universal_demo.py --config={config} model_name {model_name} model_path {model_path} input_file {file_path} save_folder {dataset_path}/sequences/{split}/{rank}"
                ],
                shell=True,
            )
