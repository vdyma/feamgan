...
from feamgan.datasetPreperation.utils.datasetUtils import (
    checkData,
    checkZipDownload,
    extractData,
)
...
def parseArguments():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        help="the relative save path of the data (default: '/data/Cityscapes')",
        nargs="?",
        default="/data/Cityscapes",
        const="/data/Cityscapes",
    )
    parser.add_argument(
        "--download_only",
        type=bool,
        help="if True, the data will only be downloaded (default: False)",
        nargs="?",
        default=False,
        const=False,
    )
    parser.add_argument(
        "--extract_only",
        type=bool,
        help="if True, the data will only be extracted, if the data exists (default: False)",
        nargs="?",
        default=True,
        const=True,
    )
...
def downloadDataset(archive_save_path, cityscapes_package_list):
    cityscapes_package_list_pbar = tqdm(
        cityscapes_package_list, desc="Downloading packages"
    )
    for package in cityscapes_package_list_pbar:
        file_names = [
            os.path.basename(x) for x in glob.glob(f"{archive_save_path}/*.zip")
        ]
        download_file = True
        if package in file_names:
            try:
                ...
                x.close()
                download_file = False
                cityscapes_package_list_pbar.write(
                    f"File {package} already exists. Skipping the download of this package."
                )
            except:
                cityscapes_package_list_pbar.write(
                    f"File {package} is corrupt removing the old file and downloading the file again..."
                )
                os.remove(f"{archive_save_path}/{package}")

        if download_file:
            cityscapes_package_list_pbar.write(
                f"Downloading the package {package} (no download progress is shown here during the download)..."
            )
            subprocess.check_call(
                ["csDownload", "-d", archive_save_path, package], shell=False
            )
...
def reformateDataset(save_path, data_types, data_type_names, splits, logger):
    ...
    for data_type, data_type_name in data_types_bdd_package_pbar:
        ...
        dir_names = [
            os.path.basename(x) for x in sorted(glob.glob(f"{save_path}/{data_type}/*"))
        ]
        for d in dir_names:
            data_types_bdd_package_pbar.write(
                f"Moving {save_path}/{data_type}/{d} to {save_path}/{d}/{data_type_name} ..."
            )
            shutil.move(
                f"{save_path}/{data_type}/{d}", f"{save_path}/{d}/{data_type_name}"
            )

        if not os.listdir(f"{save_path}/{data_type}"):
            data_types_bdd_package_pbar.write(
                f"Moved all files from {save_path}/{data_type}, deleting empty folder..."
            )
            shutil.rmtree(f"{save_path}/{data_type}")
        else:
            logger.error(
                f"There are unresolved files in {save_path}/{data_type}, something went wrong!",
                ":downloadAndExtractCityscapes",
            )
        ...
        for split in tqdm(splits, desc="Reformating subsets"):
            dir_names = [
                os.path.basename(x)
                for x in sorted(glob.glob(f"{save_path}/{split}/{data_type_name}/*"))
            ]
            sequenze_index = 1
            for d in tqdm(dir_names, desc="Reformating dirs"):
                ...
                files_in_dir = [
                    os.path.basename(x)
                    for x in sorted(glob.glob(f"{dir_with_sequences}/*"))
                ]
                ...
                for f in files_in_dir:
                    ...
                    if current_dir_seq_num is not None:
                        if dir_seq_num != current_dir_seq_num:
                            ...
                            os.mkdir(new_dir)
                            data_types_bdd_package_pbar.write(
                                f"New sequence found in current dir: {dir_with_sequences}"
                            )
                            data_types_bdd_package_pbar.write(
                                f"Moving sequence {str(sequenze_index).zfill(6)} from {dir_with_sequences} to {new_dir} ..."
                            )
                    else:
                        current_dir_seq_num = dir_seq_num
                    ...
                    shutil.move(
                        f"{dir_with_sequences}/{f}", f"{new_dir}/{new_file_name}"
                    )
                if not os.listdir(dir_with_sequences):
                    data_types_bdd_package_pbar.write(
                        f"Moved all sequences from {dir_with_sequences}, deleting empty folder..."
                    )
                    shutil.rmtree(dir_with_sequences)
                else:
                    logger.error(
                        f"There are unconverted files in {dir_with_sequences}, something went wrong!",
                        ":downloadAndExtractCityscapes",
                    )
                sequenze_index += 1
...
def downloadAndExtractCityscapes(save_path, download_only=False, extract_only=False):
    ...
    if not extract_only:
        logger.info(
            "Downloading data (this can take a very long time)...",
            ":downloadAndExtractCityscapes",
        )
        logger.info(
            f"Saving files to: {archive_save_path}", ":downloadAndExtractCityscapes"
        )
    ...
    if not download_only:
        ...
        file_paths = [
            f"{archive_save_path}/{package}" for package in cityscapes_package_list
        ]
        extractData(save_path, file_paths)
        logger.info("Extraction complete.", ":downloadAndExtractCityscapes")

        logger.info(
            "Reformating data to fit the convention...", ":downloadAndExtractCityscapes"
        )
        cleanDirs(save_path)
        ...
        for data_type_name, file_type in tqdm(
            zip(data_type_names, file_types),
            total=len(data_type_names),
            desc="Checking datatypes",
        ):
            for split, seqs, num_etries in tqdm(
                zip(splits, split_seqs, split_entries),
                total=len(splits),
                desc="Checking subsets",
            ):
                checkData(
                    data_path=f"{save_path}/{split}/{data_type_name}",
                    file_type=file_type,
                    expected_sequences=seqs,
                    expected_entries=num_etries,
                )
