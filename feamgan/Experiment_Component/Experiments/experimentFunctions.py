...
def createModel(model_config, dataset_config):
    ...
    input_module = importlib.import_module(
        "feamgan.Experiment_Component.Models." + model_config["modelClassName"]
    )
    model = getattr(input_module, model_config["modelClassName"])(
        model_config,
        dataset_config,
        input_shape=dataset_config["augmentations"]["inputDataShapeA"],
        output_shape=dataset_config["augmentations"]["inputDataShapeB"],
        sequence_length=model_config["sequenceLengths"][
...
def createTrainer(model_config):
    ...
    """
    input_module = importlib.import_module(
        "feamgan.Experiment_Component.Trainer." + model_config["trainerName"]
    )
    trainer = getattr(input_module, model_config["trainerName"])()
    return trainer


def loadInputPipeline(
    dataset_config,
    batch_size,
    sequence_length,
    device_id,
    ...
    for domain, domain_relative_path in zip(domains, domain_relative_paths):
        ...
        for data_type in domain:
            train_dataset_dirs.append(
                os.path.join(data_root, domain_relative_path, "train", data_type)
            )
            eval_dataset_dirs.append(
                os.path.join(
                    data_root, domain_relative_path, val_dataset_name, data_type
                )
            )
    ...
    for pipe in train_pipes:
        pipe.build()
    dataset["train"] = [
        DALIGenericIterator(
            train_pipe,
            [data_type],
            reader_name="train_reader",
            last_batch_policy=LastBatchPolicy.DROP,
        )
        for train_pipe, data_type in zip(train_pipes, data_types)
    ]

    eval_pipes = createPipelines(
        eval_dataset_dirs,
        augmentation_configs,
        "eval",
        pipe_type,
    ...
    for pipe in eval_pipes:
        pipe.build()
    dataset["eval"] = [
        DALIGenericIterator(
            eval_pipe,
            [data_type],
            reader_name="eval_reader",
            last_batch_policy=LastBatchPolicy.DROP,
        )
        for eval_pipe, data_type in zip(eval_pipes, data_types)
    ]

    return dataset, data_names


def trainValInferModel(
    model_config,
    model_dir,
    dataset_config,
    local_rank,
    ...
    dataset, data_names = loadInputPipeline(
        dataset_config,
        batch_size,
        sequence_length,
        local_rank,
        ...
        num_gpus,
        num_threads,
        model_config["pipeline"]["daliUseCPU"],
        seed,
        dataset_config["valDatasetName"],
    )

    model_config.update(
        {
            "inputDataTypes": dataset_config["datasetATypes"],
            "outputDataTypes": dataset_config["datasetBTypes"],
        }
    )
    ...
    if model_config["doTraining"]:
        model_suit.doTraining(
            model_config["trainingSteps"],
            model_config["evalSteps"],
            model_config["trainingEpochs"],
            model_config["evalEpochs"],
        )
