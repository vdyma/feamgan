...
from feamgan.datasetPreperation.utils.datasetUtils import checkZipDownload, extractData
...
def parseArguments():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        help="the relative save path of the data (default: '/data/BDD100k')",
        nargs="?",
        default="/data/BDD100k",
        const="/data/BDD100k",
    )
    parser.add_argument(
        "--split_into_tw_subsets",
        type=bool,
        help="specify the shoudl be split into the subsets: [rainy|snowy|clear|overcast|partly cloudy|foggy] and [daytime|night|dawn/dusk]. 'True' by default.",
        nargs="?",
        default=True,
        const=True,
    )
    parser.add_argument(
        "--skip_if_exists",
        type=bool,
        help="skipps the formating of sequences if they already exist (default: 1)",
        nargs="?",
        default=True,
        const=True,
    )
...
def getDatasetParameters(split_into_tw_subsets):
    ...
    data_types = ["videos"]
    bdd_package_list = [
        "bdd100k_videos_train_00.zip",
        "bdd100k_videos_val_00.zip",
    ...
    return (
        ...
        data_type_names,
        bdd_package_list,
        bdd_package_list_md5,
        data_num,
        nr_no_label,
        nr_no_condition_files_time,
        nr_no_condition_files_weather,
    )
...
def reformateDataset(
    ...
    data_types_bdd_package_pbar = tqdm(
        zip(data_types, data_type_names),
        total=len(data_types),
        desc="Reformating datatypes",
    )
    for data_type, data_type_name in data_types_bdd_package_pbar:
        # For each split
        undefined_time_conditions = 0
        undefined_weather_conditions = 0
        for split, nr_no_cond_t, nr_no_cond_w, nr_no_l in tqdm(
            zip(
                splits,
                nr_no_condition_files_time,
                nr_no_condition_files_weather,
            ...
            total=len(splits),
            desc="Reformating subsets",
        ):
            with open(
                f"{save_path}/bdd100k/labels/bdd{data_num}_labels_images_{split}.json"
            ) as json_file:
            ...
            data_types_bdd_package_pbar.write(
                f"Moving sequences from {frames_dir} to {save_path} ..."
            )
            ...
            for label in tqdm(labels, desc="Reformating entries"):
                ...
                scene = label["attributes"]["scene"].replace(
                    " ", ""
                )  # Just for additional information in the file name
                ...
                if split_into_tw_subsets:
                    ...
                    if (frame_condition_time == "undefined") and (
                        frame_condition_weather == "undefined"
                    ):
                        data_types_bdd_package_pbar.write(
                            "No weather/daytime conditions given for this frame. Skipping and removing this frame."
                        )
                        if data_type == "videos":
                ...
                for frame_condition in [frame_condition_time, frame_condition_weather]:
                    ...
                    if (
                        os.path.exists(
                            f"{save_path}/{frame_condition}/{split}/{data_type_name}/{str(sequenze_index[frame_condition]).zfill(6)}"
                        )
                        and skip_if_exists
                    ):
                        data_types_bdd_package_pbar.write(
                            "Sequence already exists, skipping this sample"
                        )
                        continue
                    ...
                    if seq_id != current_seq_id[frame_condition]:
                        ...
                        data_types_bdd_package_pbar.write(
                            f"New sequence found in: {frames_dir}"
                        )
                        data_types_bdd_package_pbar.write(
                            f"Moving sequence {str(sequenze_index).zfill(6)} from {frames_dir} to {new_dir} ..."
                        )
                        ...
                        if prev is None:
                            if os.path.exists(f"{frames_dir}/{f}"):
                                ...
                                for frame in videogen:
                                    skvideo.io.vwrite(
                                        f"{new_dir}/{str(sequenze_index[frame_condition]).zfill(6)}_{str(index).zfill(9)}_{frame_condition}_{scene}{n}",
                                        frame,
                                    )
                                    index += 1
                            ...
                            for file in sorted(
                                glob.glob(
                                    f"{save_path}/{prev}/{split}/{data_type_name}/{str(sequenze_index[prev] - 1).zfill(6)}/*.jpg"
                                )
                            ):
                                shutil.copy(
                                    file,
                                    f"{new_dir}/{str(sequenze_index[frame_condition]).zfill(6)}_{str(index).zfill(9)}_{frame_condition}_{scene}.jpg",
                                )
                                index += 1
                            sequenze_index[frame_condition] += 1
                    else:
            ...
            if undefined_time_conditions:
                if undefined_time_conditions == nr_no_cond_t:
                    data_types_bdd_package_pbar.write(
                        f"There were {undefined_time_conditions} files with undefined daytime in {frames_dir}. {nr_no_cond_t} files with undefined daytime are expected."
                    )
                else:
                    logger.error(
                        f"There were {undefined_time_conditions} files with undefined daytime in {frames_dir}. {nr_no_cond_t} files with undefined daytime are expected. Something went wrong!",
                        ":extractBDD",
                    )
            ...
            if undefined_weather_conditions:
                if undefined_weather_conditions == nr_no_cond_w:
                    data_types_bdd_package_pbar.write(
                        f"There were {undefined_weather_conditions} files with undefined weather in {frames_dir}. {nr_no_cond_w} files with undefined weather are expected."
                    )
                else:
                    logger.error(
                        f"There were {undefined_weather_conditions} files with undefined weather in {frames_dir}. {nr_no_cond_w} files with undefined weather are expected. Something went wrong!",
                        ":extractBDD",
                    )
            ...
            if not remaining_files:
                data_types_bdd_package_pbar.write(
                    f"Moved all sequences from {frames_dir}, deleting empty folder..."
                )
                shutil.rmtree(frames_dir)
            else:
                if len(remaining_files) is nr_no_l:
                    data_types_bdd_package_pbar.write(
                        f"There are {len(remaining_files)} remaining files with no label in {frames_dir}. {nr_no_l} files with no label are expected."
                    )
                else:
                    logger.error(
                        f"There are {len(remaining_files)} remaining files with no label in {frames_dir}. {nr_no_l} files with no label are expected. Something went wrong!",
                        ":extractBDD",
                    )
...
def extractBDD(save_path, split_into_tw_subsets, skip_existing):
    ...
    (
        ...
        nr_no_condition_files_time,
        nr_no_condition_files_weather,
    ) = getDatasetParameters(split_into_tw_subsets)
