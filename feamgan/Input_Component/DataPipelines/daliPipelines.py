...
def dali_sequence_pipeline(
    ...
    if augmentation_config["resize"]:
        if "resize" in augmentation_config["preprocessMode"]:
            images = fn.resize(
                images,
                device=dali_device,
                size=augmentation_config["dataLoadSize"],
                interp_type=augmentation_config["method"],
            )
        elif "scale_width" in augmentation_config["preprocessMode"]:
            images = fn.resize(
                images,
                device=dali_device,
                resize_x=augmentation_config["dataLoadSize"],
                interp_type=augmentation_config["method"],
            )
            ...
            images = fn.resize(
                images,
                device=dali_device,
                resize_y=augmentation_config["dataLoadSize"],
                interp_type=augmentation_config["method"],
            )
    ...
    if "crop" in augmentation_config["preprocessMode"]:
        ...
        if mode == "train":
            ...
            if "top_crop" in augmentation_config["preprocessMode"]:
                crop_pos_y = fn.random.uniform(
                    device=dali_device, range=(min_crop_top_pos, max_crop_buttom_pos)
                )
            else:
    ...
    if augmentation_config["flip"] and mode == "train":
        images = fn.flip(
            images,
            horizontal=fn.random.coin_flip(device=dali_device, probability=0.5),
            device=dali_device,
        )
