...
def parseArguments():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        help="the relative save path of the data (default: '/data/PFD')",
        nargs="?",
        default="/data/PFD",
        const="/data/PFD",
    )
    parser.add_argument(
        "--epe_baseline_path",
        type=str,
        help="the relative path to the epe baseline images (default: '/data/Baselines/epe')",
        nargs="?",
        default="/data/Baselines/from_epe/epe/frames/0",
        const="/data/Baselines/from_epe/epe/frames/0",
    )
...
def reformateDataset(
    ...
    for split, ids in tqdm(zip(splits, split_ids), desc="Reformating subsets"):
        ...
        for data_type, data_type_name, file_type in tqdm(
            zip(data_types, data_type_names, file_types),
            ...
            for file_path in tqdm(files, total=len(files), desc="Reformating files"):
                ...
                seq_id = file_name.split(".")[
                    0
                ]  # To be compatible with the rest of the pipeline
        ...
        shutil.copytree(
            f"{dataset_path}/{split}/", f"{dataset_path}/val/", dirs_exist_ok=True
        )


def extractPFD(save_path: str, epe_baseline_path: str):
    ...
    reformateDataset(
        save_path, splits, split_ids, data_types, data_type_names, file_types
    )
...
if __name__ == "__main__":
    ...
    epe_baseline_path: str = args.epe_baseline_path
