...
from feamgan.Experiment_Component.Models.Backbones.Discriminators.MultiscaleDiscriminator import (
    MultiscaleDiscriminator,
)
from feamgan.Experiment_Component.Models.Backbones.Generators.FeaMGenerator import (
    FeaMGenerator,
)
...
from feamgan.Experiment_Component.Models.Losses.gradientPenalty import (
    real_penalty,
    tee_loss,
)
from feamgan.Experiment_Component.Models.Losses.PerceptualLoss import (
    PerceptualLoss,
    vgg16,
)
from feamgan.Experiment_Component.Models.Sampling.discriminatorMask import (
    discriminatorMask,
)
from feamgan.Experiment_Component.Models.Sampling.SimilarityCrop import (
    SimilarityCrop,
    similarityCropSmallPatches,
)
from feamgan.Experiment_Component.Models.utils import modelUtils
from feamgan.utils.semanticSegmentationUtils import (
    getNumSemanticLabelIds,
    labelMapToColor,
    labelMapToOneHot,
)
...
class FeaMGAN(BaseModel):
    def __init__(
        self, model_config, dataset_config, input_shape, output_shape, sequence_length
    ):
        ...
        if self.is_train:
            ...
            if self.use_sim_crop_dis:
                ...
                self.sim_crop_dis_max_batch_size = model_config["optimization"][
                    "simCropDisMaxBatchSize"
                ]

            input_shape = [3, self.sim_crop_height, self.sim_crop_width]
            output_shape = [3, self.sim_crop_height, self.sim_crop_width]

        super(FeaMGAN, self).__init__(
            model_config, dataset_config, input_shape, output_shape, sequence_length
        )
        ...
        if self.is_train:
            ...
            if self.lr_scheduler == "stepLR":
                self.niter = model_config["optimization"]["lrScheduler"][
                    "learningRateDecayStartingStep"
                ]
                self.niter_decay = model_config["optimization"]["lrScheduler"][
                    "learningRateDecayAfterSteps"
                ]
            ...
            self.perceptual_loss_type = model_config["optimization"][
                "perceptualLossType"
            ]

            # Optimization - Discriminators
            self.c_discriminator = model_config["optimization"]["cDiscriminator"]
            self.use_discriminator_mask = model_config["optimization"][
                "useDiscriminatorMask"
            ]
        ...
        for input_type in self.input_types:
            if input_type == "frames":
                ...
                self.gen_content_nc += getNumSemanticLabelIds(
                    dataset_config["nameOfDataset"], are_train_ids=False
                )
                if self.instance:
        ...
        if self.is_train and self.sim_crop_type:
            self.simCrop = SimilarityCrop(
                self.sim_crop_height,
                self.sim_crop_width,
                self.sim_crop_type,
                sim_threshold=self.sim_crop_threshold,
                sim_model=self.netS,
            )
    ...
    def defineNetworks(self):
        ...
        if backbone_name == "FeaMGenerator":
            netG = FeaMGenerator(
                self.model_config["generators"][backbone_name],
                self.gen_content_nc,
                self.gen_output_nc,
            )
        else:
            raise NotImplementedError(
                f"Generator with model name {backbone_name} could not be recognized"
            )
        ...
        if self.is_train:
            ...
            if backbone_name == "MultiscaleDiscriminator":
                netD = MultiscaleDiscriminator(
                    self.model_config["discriminators"][backbone_name],
            ...
            if self.use_sim_crop_dis:
                d_config = copy.deepcopy(
                    self.model_config["discriminators"][backbone_name]
                )
                d_config["numD"] = d_config["numD"] - 1 if d_config["numD"] > 1 else 1
                netDSim = MultiscaleDiscriminator(
                    d_config, self.lambda_gan_feat, self.dis_input_nc
                )
        ...
        if self.is_train:
            ...
            if self.optimizer_type == "AdamW":
                self.optimizer_G = torch.optim.AdamW(
                    G_params, lr=G_lr, betas=(beta1, beta2), weight_decay=0.0001
                )
                self.optimizer_D = torch.optim.AdamW(
                    D_params, lr=D_lr, betas=(beta1, beta2), weight_decay=0.0001
                )
            else:
    ...
    def getContent(self, data, index=0, to_train_ids=False):
        ...
        if self.input_types[index] == "segmentations":
            inst = (
                data[index + 1][self.input_types[index + 1]] if self.instance else None
            )
            d = labelMapToOneHot(
                d,
                self.dataset_config["nameOfDataset"],
                to_train_ids=to_train_ids,
                inst_map=inst,
            )
        return d
    ...
    def val(self, data):
        ...
        content_g = (
            torch.cat([content[0], content[self.seg_index]], axis=1)
            if self.seg_index
            else content[0]
        )
    ...
    def forwardGenerators(self, data):
        ...
        if self.use_discriminator_mask:
            fake_earse, style_earse, earse_mask = discriminatorMask(
                [fake_image_batch, content_condition], [style[0], style_condition]
            )
            main_valid = torch.max(earse_mask).item()

            if self.use_sim_crop_dis:
                content_sim_crop, style_sim_crop = similarityCropSmallPatches(
                    [fake_image_batch, content_condition],
                    [style[0], style_condition],
                    earse_mask,
                    crop_factor_h=self.crop_factor_h,
                    crop_factor_w=self.crop_factor_w,
                    max_batch_size=self.sim_crop_dis_max_batch_size,
                )
                out.update(
                    {
                        "fake_sim_crop": content_sim_crop,
                        "style_sim_crop": style_sim_crop,
                    }
                )
            ...
            pred_fake, pred_real = self.discriminate(
                fake_earse,
                style_earse,
                fake_data_sim_d=content_sim_crop,
                real_data_sim_d=style_sim_crop,
            )
            out.update(
                {
                    "fake_erase": fake_earse,
                    "style_erase": style_earse,
                    "earse_mask": earse_mask,
                }
            )
        else:
            if self.use_sim_crop_dis:
                _, _, earse_mask = discriminatorMask(
                    [fake_image_batch, content_condition], [style[0], style_condition]
                )
                content_sim_crop, style_sim_crop = similarityCropSmallPatches(
                    [fake_image_batch, content_condition],
    ...
    def generatorLosses(self, data, pred):
        ...
        for pred_f, pred_r, key_name in zip(pred_fake, pred_real, self.dis_names):
            GAN_loss_image = (
                self.criterionGAN(pred_f[0], True, for_discriminator=False)
                * self.gan_factor
            )
                    ...
                    if k < len(pred_f):
                        ...
                        for i in range(num_D):  # for each discriminator
                            ...
                            for j in range(
                                ...
                                GAN_Feat_loss += (
                                    unweighted_loss * self.lambda_gan_feat / num_D
                                )
                        losses["genarators"]["AB"][
                            f"GAN_feat_loss_{key}_{key_name}"
                        ] = GAN_Feat_loss[0] * self.gan_factor

        if self.lambda_perceptual:
            perceptual_loss = (
                self.criterionPerceptual(target.contiguous(), fake_image.contiguous())
                if "lpips_vgg" in self.perceptual_loss_type
                else self.criterionPerceptual(fake_image, target)
            )
    ...
    def forwardDiscriminators(self, data, pred_data):
        ...
        pred_fake, pred_real = self.discriminate(
            [fake_image, content_condition],
            [style[0], style_condition],
            fake_data_sim_d=fake_sim_crop,
            real_data_sim_d=style_sim_crop,
        )
    ...
    def discriminatorLosses(self, pred):
        ...
        for pred_f, pred_r, real_img, key_name in zip(
            pred_fake, pred_real, real_imgs, self.dis_names
        ):
            pred_fake_loss_image = self.criterionGAN(
                pred_f[0], False, for_discriminator=True
            )
            pred_real_loss_image = self.criterionGAN(
                pred_r[0], True, for_discriminator=True
            )
            losses["discriminators"]["B"][f"fake_loss_image_{key_name}"] = (
                pred_fake_loss_image
            )
            losses["discriminators"]["B"][f"real_loss_image_{key_name}"] = (
                pred_real_loss_image
            )

            if self.lambdaRealPenalty > 0 and valid[key_name]:
                pred_real_loss_image_penalty, _ = tee_loss(
                    0, real_penalty(pred_real_loss_image, real_img)
                )
                losses["discriminators"]["B"][f"real_loss_image_{key_name}_penalty"] = (
                    self.lambdaRealPenalty * pred_real_loss_image_penalty
                )
    ...
    def _discriminator_discriminate(
        self, netD, fake_image, real_image, fake_condition=None, real_condition=None
    ):
        fake_concat = (
            torch.cat([fake_condition, fake_image], dim=1)
            if self.c_discriminator
            else fake_image
        )
        real_concat = (
            torch.cat([real_condition, real_image], dim=1)
            if self.c_discriminator
            else real_image
        )
    ...
    def discriminate(
        ...
        fake_data_main_d,
        real_data_main_d,
        fake_data_sim_d=None,
        real_data_sim_d=None,
    ):
        pred_fake_main, pred_real_main = self._discriminator_discriminate(
            self.netD,
            fake_data_main_d[0],
            real_data_main_d[0],
            fake_data_main_d[1],
        ...
        if self.use_sim_crop_dis and fake_data_sim_d[0].shape[0]:
            pred_fake_sim, pred_real_sim = self._discriminator_discriminate(
                self.netDSim,
                fake_data_sim_d[0],
                real_data_sim_d[0],
                fake_data_sim_d[1],
    ...
    def getImages(self, data, pred, mode):
        if data and pred:
            ...
            out = {
                f"real_{self.input_types[0]}_A": pred["content"][0].view(
                    -1, self.seq_lenght, *shape_in
                ),
                f"real_{self.output_types[0]}_B": pred["style"][0].view(
                    -1, self.seq_lenght, *shape_out
                ),
            ...
            if "segmentations" in self.input_types:
                ...
                seg_A = torch.argmax(d, keepdim=True, dim=-3).view(
                    -1, self.seq_lenght, 1, *shape_in[1:]
                )
                seg_A = labelMapToColor(
                    seg_A, self.dataset_config["nameOfDataset"], are_train_ids=False
                )
    ...
    def getSummary(self, data, model_values, pred, mode):
        ...
        if data:
            ...
            if self.verbose:
                for i, input_type in enumerate(self.input_types):
                    ...
                    if "segmentations" in input_type:
                        ...
                        if self.instance:
                            ...
                            i = (
                                i.view(
                                    -1, self.seq_lenght, 1, shape_in[-2], shape_in[-1]
                                )[0]
                                .byte()
                                .cpu()
                            )
                            summary.update(
                                {
                                    f"input_content_instance_A": wandb.Video(
                                        i, fps=1, format="gif"
                                    )
                                }
                            )
                        ...
                        d = (
                            d.view(-1, self.seq_lenght, 1, shape_in[-2], shape_in[-1])[
                                0
                            ]
                            .byte()
                            .cpu()
                        ...
                        d = formatVideo(
                            ...
                            input_type,
                            vid_index=0,
                            dataset_name=self.dataset_config["nameOfDataset"],
                        )
                    summary.update(
                        {
                            f"input_content_{input_type}_A": wandb.Video(
                                d, fps=1, format="gif"
                            )
                        }
                    )
                ...
                shape_out = (
                    self.output_shape if (mode == "train") else self.output_shape_in
                )
                for i, output_type in enumerate(self.output_types):
                    ...
                    if "segmentations" in output_type:
                        ...
                        if self.instance:
                            ...
                            i = (
                                i.view(
                                    -1, self.seq_lenght, 1, shape_out[-2], shape_out[-1]
                                )[0]
                                .byte()
                                .cpu()
                            )
                            summary.update(
                                {
                                    f"input_style_instance_B": wandb.Video(
                                        i, fps=1, format="gif"
                                    )
                                }
                            )
                    ...
                    summary.update(
                        {
                            f"input_style_{output_type}_B": wandb.Video(
                                d, fps=1, format="gif"
                            )
                        }
                    )
                ...
                summary.update(
                    {f"input_content_frames_A": wandb.Video(d, fps=1, format="gif")}
                )
        ...
        if pred:
            ...
            if mode == "train":
                ...
                out_pred = {
                    "output_fake_frames_B": wandb.Video(
                        formatVideo(
                            fake_b,
                            self.output_types[0],
                            vid_index=0,
                ...
                for i in range(0, len(pred["fake_B_batch"])):
                    ...
                    out_pred = {
                        f"output_fake_frames_B.{i}": wandb.Video(
                            formatVideo(
                                fake_b,
                                self.output_types[0],
                                vid_index=0,
            ...
            if self.verbose and (mode == "train"):
                if self.use_discriminator_mask:
                    ...
                    fake_b_erase = pred["fake_erase"][0].view(
                        -1, self.seq_lenght, *erase_shape
                    )
                    style_b_erase = pred["style_erase"][0].view(
                        -1, self.seq_lenght, *erase_shape
                    )
                    out_pred = {
                        "output_fake_frames_B_erase": wandb.Video(
                            formatVideo(
                                fake_b_erase,
                                self.output_types[0],
                                vid_index=0,
                        ...
                        "input_style_frames_B_erase": wandb.Video(
                            formatVideo(
                                style_b_erase,
                                self.input_types[0],
                                vid_index=0,
