...
def createBDDSubsets(
    project_path,
    bdd_subset,
    dataset_defintion_txt_path_train,
    dataset_defintion_txt_path_val,
):
    ...
    for dataset_definition_path_txt, split in zip(
        [dataset_defintion_txt_path_train, dataset_defintion_txt_path_val],
        ["train", "val"],
    ):
        ...
        for subset_path in tqdm(subset_paths):
            ...
            subset_path_seg = f"segmentations_mseg/{subset_path}".replace(
                ".jpg", ".png"
            )
    ...
    logger.info(
        f"Creating BDD100k subsets for {bdd_subset} complete.", ":createBDDSubsets"
    )
...
if __name__ == "__main__":
    for bdd_subset in ["daytime", "night", "clear", "snowy"]:
        dataset_defintion_txt_path_train = (
            f"feamgan/datasetPreperation/BDDSubsets/bdd_{bdd_subset}_train.txt"
        )
        dataset_defintion_txt_path_val = (
            f"feamgan/datasetPreperation/BDDSubsets/bdd_{bdd_subset}_val.txt"
        )
