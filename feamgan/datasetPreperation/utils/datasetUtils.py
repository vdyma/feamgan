...
def extractData(extract_path: str, file_paths: list[str]):
...
def checkZipDownload(download_path, expected_file_names, md5_list=None, a_type=".zip"):
    ...
    if md5_list:
        for fn, md5 in tqdm(
            zip(expected_file_names, md5_list),
            total=len(expected_file_names),
            desc="Checking md5 sums",
        ):
            f = Path(f"{download_path}/{fn}")
            check_sum = hashlib.md5(open(f, "rb").read()).hexdigest()
            if check_sum != md5:
                logger.error(
                    f"Md5 check sum does not match. File {f} is corrupted!",
                    ":checkZipDownload",
                )

    for fn in tqdm(
        expected_file_names, total=len(expected_file_names), desc="Checking zip files"
    ):
        ...
        if not f.is_file():
            logger.error(
                f"Error: {a_type} file {f} does not exist!", ":checkZipDownload"
            )
            continue
        try:
            if a_type == ".zip":
...
def checkAndPrintFoundExpected(found: int, expected: int, name: str) -> bool:
    logger = SLoggerHandler().getLogger(LoggerNames.INPUT_C)
    if found != expected:
        logger.error(
            f"Number of {name} not as expected!", ":checkAndPrintFoundExpected"
        )
...
def checkData(
    ...
    checkAndPrintFoundExpected(
        num_entries, expected_entries, f"entries from type {file_type} in {data_path}"
    )
