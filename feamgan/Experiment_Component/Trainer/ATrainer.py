...
class ATrainer(ITrainer):
    ...
    def discriminator(self, data, fake_data, model):
        if self.mode == "train":
            ...
            for dis in discriminators:
                for d in dis:
                    self.toggleGrad(d, True)
            for gen in generators:
                self.toggleGrad(gen, False)

            for opt in discriminators_opts:
                opt.zero_grad()
        ...
        if self.mode == "train":
            ...
            for l, opt, dis in zip(
                loss, discriminators_opts, discriminators
            ):  # , discriminators):
    ...
    def generator(self, data, model):
        if self.mode == "train":
            ...
            for dis in discriminators:
                for d in dis:
                    self.toggleGrad(d, False)
            for gen in generators:
                self.toggleGrad(gen, True)

            for opt in generator_opts:
                opt.zero_grad()
