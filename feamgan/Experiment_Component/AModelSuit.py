...
from feamgan.utils.visualizationUtils import convertTime, formatFramesToUnit
...
class AModelSuit(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self._model_dir = (
            f"experimentResults{model_dir}"
            if "pretrainedModels/" not in model_dir
            else model_dir
        )
        ...
        if "train" in self._dataset:
            train_iter_lens = [
                it._size // self._batch_size for it in self._dataset["train"]
            ]
            main_train_iter_len = train_iter_lens[0]
        if "eval" in self._dataset:
            eval_iter_lens = [
                it._size // self._batch_size for it in self._dataset["eval"]
            ]
            main_eval_iter_len = eval_iter_lens[0]

        if self._local_rank == 0:
            self._logger.train(
                f"Training Iterator lengths are: {train_iter_lens}",
                "AModelSuit:__init__",
            )
            self._logger.val(
                f"Validation Iterator lengths are: {eval_iter_lens}",
                "AModelSuit:__init__",
            )
    ...
    def _initModel(self):
        ...
        if self._model.is_train:
            ...
            self._model, self._model.optimizers = amp.initialize(
                self._model,
                self._model.optimizers,
                opt_level=self._amp_opt_level,
                num_losses=len(self._model.optimizers),
            )
    ...
    def _getNextIteratorData(self, mode):
        ...
        for it in self._dataset[mode]:
            try:
                ...
                if self._local_rank == 0:
                    self._logger.info(
                        f"Iterator reseted before main interator as expected, continue {mode} in this epoch",
                        "AModelSuit:doValidation",
                    )
                d = it.next()
            data.append(d[0])
        ...
        for i, it in enumerate(self._dataset[mode]):
            ...
            if it._size // self._batch_size != main_iter_len:
                if dry_run_unfinished_iterators:  # For validation we reset the iterators at the end of validation to keep the val set the same
                    if self._local_rank == 0:
                        self._logger.info(
                            f"Main iterator length is {main_iter_len}. Dry run iterator {i} with incompatible length {it._size // self._batch_size}",
                            "AModelSuit:resetIterators",
                        )
                    for d in it:
                        continue
                    it.reset()
                    if self._local_rank == 0:
                        self._logger.info(
                            f"Reseted iterator {i} with incompatible length {it._size // self._batch_size}",
                            "AModelSuit:resetIterators",
                        )
                else:  # For training we reset the main iterator at the endo of a epoch and the incompatible iterator when it is finished
                    if self._local_rank == 0:
                        self._logger.info(
                            f"Iterator {i} with length {it._size // self._batch_size} not finished, ignoring reset",
                            "AModelSuit:resetIterators",
                        )
            else:
                it.reset()
                if self._local_rank == 0:
                    self._logger.info(
                        f"Reseted iterator {i} with compatible length {it._size // self._batch_size}",
                        "AModelSuit:resetIterators",
                    )
    ...
    def _train_epoch(self, train_steps, eval_steps, end_train_log_time):
        ...
        while inter_len > train_epoch_step:
            ...
            self._train_load_batch_time_meter.update(
                time.time() - end_train_load_batch_time
            )
            ...
            for _ in range(self._trainer.getDiscriminatorTrainingSteps()):
                dis_losses, dis_pred = self._trainer.discriminator(
                    data, gen_pred, self._model
                )
            self._train_model_batch_time_meter.update(
                time.time() - end_train_model_batch_time
            )

            self._state["current_model_values"]["train"] = {
                "generator_losses": gen_losses,
                "discriminator_losses": dis_losses,
            }
            ...
            if (
                (self._local_rank == 0)
                and (self._save_checkpoint_steps > 0)
                and (cs % self._save_checkpoint_steps == 0)
            ):
            ...
            if (self._save_summary_steps > 0) and (cs % self._save_summary_steps == 0):
                metrics_values = self._model.module.updateMetrics(
                    data, gen_pred, "train"
                )
                metrics_values = self._model.module.reduceMetrics(
                    [metrics_values], "train", self._num_gpus
                )
            ...
            if (self._log_steps > 0) and (cs % self._log_steps == 0):
                self._train_log_steps_time_meter.update(
                    time.time() - end_train_log_time
                )
                end_train_log_time = time.time()
                self._updateTimes()
                if self._local_rank == 0:
                    self._logger.train(
                        f"Step {cs} of {train_steps}. Epoch step {train_epoch_step} of {inter_len}. Name: {self._model_name}",
                        "AModelSuit:_train",
                    )

            # Capture the batch time
            self._train_batch_time_meter.update(
                time.time() - end_train_batch_time - eval_time_subtract
            )
            self._updateTimes()
            ...
            if train_steps >= 0:
                if cs >= train_steps:
                    self._logger.train(
                        f"Finished training at step {cs} in epoch {ce} for {self._model_name}",
                        "AModelSuit:_train",
                    )
    ...
    def doTraining(self, train_steps, eval_steps, train_epochs=-1, eval_epochs=-1):
        ...
        while not self._state["training_done"]:
            ...
            if first_run:
                ...
                passed_eval_time, data, pred = self._train_epoch(
                    train_steps, eval_steps, end_train_log_time
                )
            ...
            if (
                eval_epochs > 0 and ce % eval_epochs == 0 and consider["validation"]
            ) or (self._state["training_done"] and eval_epochs > 0):
                self.doValidation("eval")
                self._trainer.train()
                self._model.train()
                eval_time_subtract = self._eval_epoch_time_meter.val
            ...
            if self._local_rank == 0:
                if (
                    self._save_checkpoint_epochs > 0
                    and ce % self._save_checkpoint_epochs == 0
                    and consider["checkpoint"]
                ) or self._state["training_done"]:
                    self._saveCheckpoint()

            # If a summary should be saved and save_summary_epochs epochs past, save the summary.
            if (
                self._save_summary_epochs > 0
                and ce % self._save_summary_epochs == 0
                and consider["summary"]
            ) or self._state["training_done"]:
            ...
            if (
                self._log_epochs > 0 and ce % self._log_epochs == 0 and consider["log"]
            ) or self._state["training_done"]:
                if ce > 0:
                    self._train_log_steps_time_meter.update(
                        time.time() - end_train_log_time
                ...
                if self._local_rank == 0:
                    ...
                    self._logger.train(
                        f"Epoch {ce} of {train_epochs}. Name: {self._model_name}",
                        "AModelSuit:doTraining",
                    )
            ...
            self._train_epoch_time_meter.update(
                time.time() - end_epoch_train_time - eval_time_subtract
            )
            end_epoch_train_time = time.time()
            self._updateTimes()
        ...
        if self._local_rank == 0:
            self._logger.train(
                f"Finished training for {train_epochs} epochs or {train_steps} steps for {self._model_name}. Evaluation was every {eval_steps} steps and/or {eval_epochs} epochs.",
                "AModelSuit:doTraining",
            )
        ...
        if (eval_epochs > 0) or (eval_steps > 0):
            ...
            self._logger.train(
                f"Starting extended evaluation...", "AModelSuit:doTraining"
            )
    ...
    def doValidation(self, mode, extended_validation=False):
        ...
        if self._local_rank == 0:
            self._logger.val(
                f"Started validation for {mode} dataset.", "AModelSuit:doValidation"
            )
        ...
        while inter_len > val_step:
            ...
            self._eval_load_batch_time_meter.update(
                time.time() - end_eval_load_batch_time
            )
            ...
            with torch.no_grad():
                ...
                self._eval_model_batch_time_meter.update(
                    time.time() - end_eval_model_batch_time
                )
                val_step += 1
                data = self._trainer.endOfStep(data, None, None, self._model)

            torch.cuda.synchronize()
            overall_metrics_values.append(
                self._model.module.updateMetrics(
                    data, gen_pred, mode, extended_validation
                )
            )
            ...
            if (self._log_steps >= 0) and (val_step % self._log_steps == 0):
                ...
                if self._local_rank == 0:
                    self._logger.val(
                        f"Validation Step {val_step} of {inter_len} for mode {mode}. Name: {self._model_name}",
                        "AModelSuit:_val",
                    )
        ...
        mean_eval_model_values = self._model.module.reduceMetrics(
            overall_metrics_values, mode, self._num_gpus, extended_validation
        )
        ...
        if self._local_rank == 0:
            ...
            self._logger.val(
                f"Finished validation for {mode} dataset. Name: {self._model_name}",
                "AModelSuit:doValidation",
            )
    ...
    def doInference(self, mode):
        ...
        if self._local_rank == 0:
            self._logger.val(
                f"Started inference for {mode} dataset.", "AModelSuit:doInference"
            )
        ...
        self._logger.val(
            f"Validation Iterator lenghts are: {eval_iter_lens}",
            "AModelSuit:doInference",
        )
        ...
        data = None
        while (
            inter_len > infer_step
        ):  # Some iterator reset before the main interator [0], therefore we want to continue evaluation
            data = self._getNextIteratorData(mode)
            ...
            with torch.no_grad():
                ...
                infer_gen_batch_time_meter.update(
                    time.time() - end_infer_gen_batch_time
                )
                infer_step += 1
                data = self._trainer.endOfStep(data, None, None, self._model)
        ...
        if self._local_rank == 0:
            self._logger.val(
                f"Finished inferece for {mode} dataset.\nAvg batch time: {convertTime(infer_gen_batch_time_meter.avg)}.\nInference time: {convertTime(infer_time)}",
                "AModelSuit:doInference",
            )
    ...
    def _saveCheckpoint(self):
        ...
        latest_checkpoint_path = os.path.join(
            self._model_dir, "checkpoints", latest_checkpoint_name
        )
        ...
        self._logger.train(
            f"\nSaved checkpoint for epoch {ce} step {cs} at {fn}. Name: {self._model_name}",
            "AModelSuit:_saveCheckpoint",
        )
    ...
    def _loadCheckpoint(self, load_checkpoint):
        ...
        if load_checkpoint is not None:
            if load_checkpoint == "latest":
                cp_path = os.path.join(
                    self._model_dir, "checkpoints", "latest_checkpoint.txt"
                )
        ...
        if restore_checkpoint:
            checkpoint_path = os.path.join(
                self._model_dir, "checkpoints", restore_checkpoint
            )
            self._loaded_checkpoint = torch.load(
                checkpoint_path,
                map_location=lambda storage, loc: storage.cuda(self._local_rank),
            )
            self._model.load_state_dict(
                self._loaded_checkpoint["model_state_dict"], strict=False
            )
            if self._model.module.is_train:
                self._model.module.loadOptimizerStateDict(
                    self._loaded_checkpoint["optimizer_state_dict"]
                )
            ...
            if self._local_rank == 0:
                self._logger.info(
                    f"Restored model from {checkpoint_path}",
                    "AModelSuit:loadCheckpoint",
                )
            ...
            Path(os.path.join(self._model_dir, "checkpoints")).mkdir(
    ...
    def _saveSummary(self, data, out_values, mode):
        ...
        model_summary = self._model.module.getSummary(
            data, self._state["current_model_values"][mode], out_values, mode
        )
    ...
    def _updateTimes(self):
        ...
        self._state["avg_load_batch_time"]["train"] = (
            self._train_load_batch_time_meter.avg
        )
        self._state["avg_load_batch_time"]["eval"] = (
            self._eval_load_batch_time_meter.avg
        )
        self._state["avg_model_batch_time"]["train"] = (
            self._train_model_batch_time_meter.avg
        )
        self._state["avg_model_batch_time"]["eval"] = (
            self._eval_model_batch_time_meter.avg
        )
        ...
        self._state["avg_log_steps_time"]["train"] = (
            self._train_log_steps_time_meter.avg
        )
    ...
    def createVideoFromInference(self, mode, dataset_config, num_frames):
        ...
        if self._local_rank == 0:
            self._logger.val(
                f"Started creating video from inference for {mode} subset and {self._model_name}.",
                ":createVideoFromInference",
            )

            data_types_A = dataset_config["datasetATypes"]
            dataset_dirs = [
                os.path.join(self._model_dir, "inference", mode, f"real_{real_A}_A")
                for real_A in data_types_A
            ]
            dataset_dirs.extend(
                [
                    os.path.join(self._model_dir, "inference", mode, f"real_frames_B"),
                    os.path.join(self._model_dir, "inference", mode, f"fake_frames_B"),
                ]
            )
            ...
            for dataset_dir in dataset_dirs:
                ...
                jpgs.sort(
                    key=lambda x: "".join(x.split(f"/{name}/")[-1].split("_")[1:])
                )
            ...
            if self._use_wandb:
                summary = {
                    f"{name}_video_1fps": wandb.Video(frame_list, fps=1, format="mp4"),
                    f"{name}_video_10fps": wandb.Video(
                        frame_list, fps=10, format="mp4"
                    ),
                }
                wandb.log(summary, step=self._state["current_step"])

            self._logger.val(
                f"Finished creating video from inference for {mode} subset and {self._model_name}.",
                ":createVideoFromInference",
            )
