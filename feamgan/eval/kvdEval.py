...
def parseArguments():
    ...
    parser.add_argument(
        "--model_name",
        type=str,
        help="the name of the model to evaluate (default: 'model')",
        nargs="?",
        default="FeaMGAN_PFD_to_CS_Crop352_Full_r1",
        const="FeaMGAN_PFD_to_CS_Crop352_Full_r1",
    )
    parser.add_argument(
        "--dataset_A_name",
        type=str,
        help="the name of dataset_A_name, the dataset of the spurce images (default: pfd)",
        nargs="?",
        default="pfd",
        const="pfd",
    )
    parser.add_argument(
        "--dataset_B_name",
        type=str,
        help="the name of dataset_B_name, the dataset of the target images (default: Cityscapes)",
        nargs="?",
        default="Cityscapes",
        const="Cityscapes",
    )
    parser.add_argument(
        "--subset",
        type=str,
        help="the subset to evalaute (default: eval)",
        nargs="?",
        default="eval",
        const="eval",
    )
    parser.add_argument(
        "--repetition",
        type=int,
        help="the training repetition to evalaute (default: 0)",
        nargs="?",
        default=0,
        const=0,
    )
    parser.add_argument(
        "--model_path",
        type=str,
        help="if model_path is specified, the metrics will be calculated for the model found in model_path (default: 0)",
        nargs="?",
        default="",
        const="",
    )
    parser.add_argument(
        "--segmentations_path",
        type=str,
        help="segmentations_path of mseg_segmentations (default: 0)",
        nargs="?",
        default="",
        const="",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        help="the batch size to use for metric calculation (default: 1)",
        nargs="?",
        default=1,
        const=1,
    )
    parser.add_argument(
        "--num_threads",
        type=int,
        help="the nummer of threads to use for metric calculation  (default: 255)",
        nargs="?",
        default=255,
        const=255,
    )
    parser.add_argument(
        "--data_load_height",
        type=int,
        help="before metric calculation the images height is rescaled to data_load_height, the aspect ration is kept the same  (default: 526)",
        nargs="?",
        default=526,
        const=526,
    )
    parser.add_argument(
        "--crop_size_h",
        type=int,
        help="before metric calculation the images are center cropped to the given cropsize (default: 526)",
        nargs="?",
        default=526,
        const=526,
    )
    parser.add_argument(
        "--crop_size_w",
        type=int,
        help="before metric calculation the images are center cropped to the given cropsize (default: 957)",
        nargs="?",
        default=957,
        const=957,
    )
    parser.add_argument(
        "--metric_parts",
        type=list,
        help="calculating the metric over the entire dataset is memory consuming, therefore the metric is calculated in parts. (default: ['vgg16_f_ll'])",
        nargs="?",
        default=["vgg16_f_ll"],
        const=["vgg16_f_ll"],
    )
    parser.add_argument(
        "--eval_steps",
        type=int,
        help="the evaluation will performed for the specified number of steps, if eval_steps is given. If eval_steps is None the evaluation will be performed for the entire dataset. (default: None)",
        nargs="?",
        default=0,
        const=0,
    )
    parser.add_argument(
        "--every_x_steps",
        type=int,
        help="the evaluation will performed every x steps, if every_x_steps is given. If every_x_steps is None the evaluation will be performed for the entire dataset. (default: None)",
        nargs="?",
        default=1,
        const=1,
    )
    parser.add_argument(
        "--shuffel",
        type=bool,
        help="if True, the datasets are shuffeld for metric calculation, this is recommended if eval_steps is given. (default: 0)",
        nargs="?",
        default=0,
        const=0,
    )
    parser.add_argument(
        "--metric",
        type=str,
        help="metric: sKVD or CSKID (default: sKVD)",
        nargs="?",
        default="sKVD",
        const="sKVD",
    )
...
def calculate_metric(
    input_frames1,
    input_segmentations1,
    input_frames2,
    input_segmentations2,
    batch_size,
    num_threads,
    dataset_name,
    data_load_height,
    ...
    for metric_part in metric_parts:
        ...
        dataset = {
            mode: [
                ...
                for pipe, data_type in zip(pipes, data_types)
            ]
        }
        ...
        if metric == "sKVD":
            eval_metric = SkvdMetric(
                is_video=False,
                model_dir="",
                dataset_name=dataset_name,
                dis_model_name=metric_part,
            )
            logger.val(f"Calculating {metric} ...", ":metricEval")
        elif metric == "CSKID":
            eval_metric = CskidMetric(
                is_video=False,
                model_dir="",
                dataset_name=dataset_name,
                dis_model_name=metric_part,
            ...
            for it in dataset[mode]:
                try:
                    ...
                    print(
                        ...
                        "calculate_skvd:getNextIteratorData",
                    )
                    d = it.next()
                data.append(d[0])
            return data
        ...
        while iter_len > step:
            ...
            if not step % every_x_steps:
                ...
                with torch.no_grad():
                    eval_metric.forwardBatch(
                        [
                            data[0]["frames"].squeeze(0),
                            data[1]["segmentations_mseg"].squeeze(0),
                        ],
                        [
                            data[2]["frames"].squeeze(0),
                            data[3]["segmentations_mseg"].squeeze(0),
                        ],
                        mode,
                    )
        ...
        logger.val(
            f"Performed {performed_evaluations} evaluation steps, reducing batches ...:",
            ":metricEval",
        )
...
def metricEval(
    ...
    if model_path:
        ...
        if "from_epe" in model_path:
            ...
            input_segmentations_dir_1 = (
                f"{project_path}/data/Baselines/{model_path}/segmentations_mseg"
            )
        else:
        ...
        logger.val(
            f"Evaluating {model_name} repetition {repetition} on {subset} set of {dataset_A_name}...",
            ":metricEval",
        )
    ...
    input_segmentations_dir_2 = (
        f"{project_path}/data/{dataset_B_name}/sequences/val/segmentations_mseg"
    )
