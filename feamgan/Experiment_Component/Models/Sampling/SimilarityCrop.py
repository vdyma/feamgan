...
@torch.no_grad()
def _similarityCropVgg(
    image1, image2, seg1, seg2, shape, height, width, sim_threshold, sim_model
):
    best_top1 = torch.randint(0, shape[-2] - height, (1,))
    best_left1 = torch.randint(0, shape[-1] - width, (1,))

    crops1 = image1[
        :, :, best_top1 : best_top1 + height, best_left1 : best_left1 + width
    ]
...
@torch.no_grad()
def _similarityCropSegSimRandomResize(
    image1, image2, seg1, seg2, shape, height, width, sim_threshold, sim_model
):
    sim_content = torch.eq(seg1, seg2)
...
class SimilarityCrop(nn.Module):
    def __init__(
        self, height, width, similarity_type="vgg", **kwargs
    ):  # sim_threshold=0.5, sim_model
        super().__init__()
        self.h = height
        self.w = width
        self.kwargs = kwargs
        ...
        if similarity_type == "random":
            ...
            self.resize_frames = transforms.Resize(
                size=(self.h, self.w), interpolation=InterpolationMode.BICUBIC
            )
            self.resize_other = transforms.Resize(
                size=(self.h, self.w), interpolation=InterpolationMode.NEAREST
            )
    ...
    def forward(self, inputs1, inputs2):
        ...
        for b in range(shape[0]):
            if self.similarity_type == "segSimRS":
                best_top1, best_left1, best_top2, best_left2, best_h, best_w = (
                    self.sim_fn(
                        inputs1[0][b],
                        inputs2[0][b],
                        inputs1[1][b],
                ...
                f_img = inputs1[0][
                    ...
                    :,
                    :,
                    best_top1 : best_top1 + best_h,
                    best_left1 : best_left1 + best_w,
                ]
                ...
                best_top1, best_left1, best_top2, best_left2 = self.sim_fn(
                    inputs1[0][b],
                    inputs2[0][b],
                    inputs1[1][b],
                    inputs2[1][b],
                    ...
                    crops1[i].append(
                        torch.unsqueeze(
                            img[
                                ...
                                :,
                                :,
                                best_top1 : best_top1 + self.h,
                                best_left1 : best_left1 + self.w,
                            ],
                            0,
                        )
                    )
                    ...
                    crops2[i].append(
                        torch.unsqueeze(
                            img[
                                ...
                                :,
                                :,
                                best_top2 : best_top2 + self.h,
                                best_left2 : best_left2 + self.w,
                            ],
                            0,
                        )
                    )
...
def similarityCropSmallPatches(
    inputs1,
    inputs2,
    erase_mask,
    crop_factor_h=8,
    ...
    erase_mask = (
        erase_mask.unfold(-2, h, h)
        .unfold(-2, w, w)
        .permute(1, 0, 2, 3, 4, 5)
        .reshape(1, shape[0] * crop_factor_w * crop_factor_h, h, w)
        .permute(1, 0, 2, 3)
    )
    ...
    inputs1 = [
        ...
        .unfold(-2, w, w)
        .permute(1, 0, 2, 3, 4, 5)
        .reshape(x.shape[1], shape[0] * crop_factor_w * crop_factor_h, h, w)
        .permute(1, 0, 2, 3)
        for x in inputs1
    ]
    inputs2 = [
