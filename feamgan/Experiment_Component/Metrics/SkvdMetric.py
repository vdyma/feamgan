...
class SkvdMetric(KidMetric):
    def __init__(self, is_video, model_dir, dataset_name, dis_model_name="vgg16_f_ll"):
        super(SkvdMetric, self).__init__(
    ...
    def forwardBatch(self, real_data, fake_data, mode, save_real_prefix=None):
        ...
        if meter_key not in self.meters:
            self.meters[meter_key] = {
                "real_des": [],
                "real_act": [[] for _ in range(len(fake_activations))],
                "fake_des": [],
                "fake_act": [[] for _ in range(len(fake_activations))],
            }
    ...
    def _getMatchingPairs(
        self, fake_des, fake_act, real_des, real_act, sim_threshold=0.5
    ):
        self.nr_no_matching_pairs = 0
    ...
    def reduceBatches(self, mode, save_real_prefix=None):
        ...
        if meter_key in self.meters:
            if save_real_prefix and os.path.exists(path) and (mode != "train"):
                ...
                if self.meters[meter_key]["real_des"]:
                    ...
                    if isMaster():
                        ...
                        real_act = [
                            torch.cat(act).cpu().data.numpy() for act in real_act
                        ]
                    if save_real_prefix:
            ...
            if self.meters[meter_key]["fake_des"]:
                ...
                if isMaster():
                    ...
                    fake_activations_paired, real_activations_paired = (
                        self._getMatchingPairs(fake_des, fake_act, real_des, real_act)
                    )
                    kids = {}
                    i = 1
            ...
            self.meters[meter_key] = {
                "real_des": [],
                "real_act": [
                    [] for _ in range(len(self.meters[meter_key]["fake_act"]))
                ],
                "fake_des": [],
                "fake_act": [
                    [] for _ in range(len(self.meters[meter_key]["fake_act"]))
