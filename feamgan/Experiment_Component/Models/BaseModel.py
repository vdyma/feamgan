...
class BaseModel(torch.nn.Module):
    ...
    def initMetrics(self):
        self.fid_metric_frame = FidMetric(
            is_video=False,
            model_dir=self.model_dir,
            dataset_name=self.dataset_config["nameOfDataset"],
            dis_model_name="inception_v3",
        )
        self.kid_metric_frame = KidMetric(
            is_video=False,
            model_dir=self.model_dir,
            dataset_name=self.dataset_config["nameOfDataset"],
            dis_model_name="inception_v3",
        )
        self.skvd_metric_frame = SkvdMetric(
            is_video=False,
            model_dir=self.model_dir,
            dataset_name=self.dataset_config["nameOfDataset"],
            dis_model_name="vgg16_f_ll",
        )
    ...
    def updateMetrics(self, data, pred, mode, extended_validation=False):
        ...
        if data and pred:
            ...
            if mode is "eval":
                ...
                self.skvd_metric_frame.forwardBatch(
                    [fake_b, fake_seg_b], [real_b, real_seg_b], mode
                )
    ...
    def reduceMetrics(self, metrics_values, mode, num_gpus, extended_validation=False):
        ...
        if mode is "eval":
            ...
            return {
                "fid_B_frame": fid_B_frame,
                "kid_B_frame": kid_B_frame,
                "skvd_frame": skvd_frame,
            }
