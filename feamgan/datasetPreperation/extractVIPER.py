...
def parseArguments():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        help="the relative save path of the data (default: '/data/VIPER')",
        nargs="?",
        default="/data/VIPER",
        const="/data/VIPER",
    )
    parser.add_argument(
        "--training_data_only",
        type=bool,
        help="if True, only the training data will be downloaded (default: False)",
        nargs="?",
        default=False,
        const=False,
    )
    parser.add_argument(
        "--evaluation_data_only",
        type=bool,
        help="if True, only the evaluation data will be downloaded (default: False)",
        nargs="?",
        default=False,
        const=False,
    )
    parser.add_argument(
        "--remove_unwanded_sequences",
        type=bool,
        help="if True, unwanted sequences for viper to cityscapes transfer (like night) will be removes (default: False)",
        nargs="?",
        default=True,
        const=True,
    )
...
def reformateDataset(
    dataset_path, splits, data_types, data_type_names, unwanted_sequences
):
    for split in tqdm(splits, desc="Reformating subsets"):
        for data_type, data_type_name in tqdm(
            ...
            shutil.move(
                f"{dataset_path}/{split}/{data_type}",
                f"{dataset_path}/{split}/{data_type_name}",
            )

            if unwanted_sequences[split]:
                frame_dirs = sorted(
                    glob.glob(f"{dataset_path}/{split}/{data_type_name}/*/")
                )
...
def extractVIPER(
    ...
    splits, file_types, data_types, data_type_names, split_seqs, split_entries = (
        getDatasetParameters(
            training_data_only, evaluation_data_only, unwanted_sequences
        )
    )
...
if __name__ == "__main__":
    ...
    extractVIPER(
        save_path, training_data_only, evaluation_data_only, unwanted_sequences
    )
